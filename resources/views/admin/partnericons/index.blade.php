        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4>Xamkorlarimiz Ikonkalari</h4>
                </div>
                @if (session('success1'))
                    <center>
                ...
                @can('partnericon-create')
                    <a href="{{ route('admin.partnericons.create') }}" style="margin-left: 25px"
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped" id="table-1">
                            ...
                            <tbody>
                                ...
                                @foreach ($partnericons as $partnericon)
                                    <tr style="text-align-last: center">
                                        <th>{{ $loop->iteration }}</th>
                                        <td><img src="{{ $partnericon->icon }}" width="150"></td>
                                        <th>{{ $partnericon->url }}</th>
                                        <td>{{ $partnericon->created_at->format('Y-m-d'); }}</td>
                                        <td>{{ $partnericon->updated_at->format('Y-m-d'); }}</td>
                                        <td style="width: 50px">
                                            @can('partnericon-edit')
                                                <a style="margin-top: 15px" class="btn btn-info btn btn-block btn-sm"
                                                    href="{{ route('admin.partnericons.edit', $partnericon->id) }}">
                                                    <i class="far fa-edit text-white"></i></a>
                                            @endcan
                                            @can('partnericon-list')
                                                <a class="btn btn-secondary btn btn-block  btn-sm"
                                                    href="{{ route('admin.partnericons.show', $partnericon->id) }}">
                                                    <i class="fas fa-eye text-white"></i></a>
                                            @endcan
                                            @can('partnericon-delete')
                                                <form action="{{ route('admin.partnericons.destroy', $partnericon->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
